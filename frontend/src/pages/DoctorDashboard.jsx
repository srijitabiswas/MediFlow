...
export default function DoctorDashboard() {
  const {
    doctors, queues, updateQueue, completeConsultation, markDelay, addEmergency,
    addToast, setRole, backendOnline, subscribeDoctorRoom,
  } = useApp();
  const navigate = useNavigate();

  useEffect(() => { setRole('doctor'); }, [setRole]);

  const [activeId, setActiveId] = useState(doctors[0]?.id ?? null);

  // Keep the selection valid as the doctors list loads/changes
  useEffect(() => {
    if (!doctors.length) return;
    const stillExists = doctors.some((d) => String(d.id) === String(activeId));
    if (!stillExists) setActiveId(doctors[0].id);
  }, [doctors, activeId]);

  // Join this doctor's live Socket.io room — no-op in demo mode
  useEffect(() => {
    if (activeId != null) subscribeDoctorRoom(activeId);
  }, [activeId, subscribeDoctorRoom]);

  const activeDoc  = doctors.find((d) => String(d.id) === String(activeId)) || doctors[0];
  const queue      = (activeDoc && queues[activeDoc.id]) || [];
  ...
  // NOTE: "Start Next" only advances the local/visual queue — the backend
  // automatically promotes the next patient to "consulting" whenever
  // completeConsultation() is called, so this purely covers the rare edge
  // case of the very first patient of the day with nobody consulting yet.
  function startNext() {
    updateQueue(activeDoc.id, (q) => q.map((p, i) => ({ ...p, status: i === 0 ? 'consulting' : 'waiting' })));
    ...
    <div className="page-wrap-lg">
      ...
      <div className="flex gap-2.5 mb-6 overflow-x-auto pb-2">
        {doctors.map((doc) => {
          ...
          const active = String(activeDoc.id) === String(doc.id);
          return (
            <button
              key={doc.id}
        ...
        <div>
          ...
          {consulting ? (
            <div className="rounded-2xl p-6 border-2 mb-6" style={{ background: '#E1F5EE', borderColor: '#0F6E56' }}>
              ...
              <button
                onClick={() => completeConsultation(activeDoc.id)}
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-100 flex justify-between items-center">
              ...
              <span className="text-xs font-bold text-gray-500">
                {waiting.length} waiting · {(activeDoc.patientsToday || 0) - (activeDoc.patientsLeft || 0)} done
              </span>
            </div>
            {waiting.length === 0 ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="card p-5">
            ...
            <div className="flex flex-col gap-2.5">
              <button
                onClick={() => markDelay(activeDoc.id, 5)}
              ...
              <button
                onClick={() => markDelay(activeDoc.id, 10)}
                className="p-3.5 rounded-xl border-2 text-left font-bold text-sm cursor-pointer transition-colors"
              ...
              <button
                onClick={() => addEmergency(activeDoc.id)}
                className="p-3.5 rounded-xl border-2 text-left font-bold text-sm cursor-pointer transition-colors"
          ...
          <div className="card p-5">
            ...
            {[
              ...
              ['Completed',       (activeDoc.patientsToday || 0) - (activeDoc.patientsLeft || 0), '#166534'],
            ...
            <div className="mt-3.5">
              <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                ...
                <span>{Math.round((((activeDoc.patientsToday || 0) - (activeDoc.patientsLeft || 0)) / (activeDoc.patientsToday || 1)) * 100)}%</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className="h-2 rounded-full transition-all duration-500"
                  style={{
                    width: `${(((activeDoc.patientsToday || 0) - (activeDoc.patientsLeft || 0)) / (activeDoc.patientsToday || 1)) * 100}%`,
