...
export default function QueueTracking() {
  const { myBooking, doctors, queues, addToast, setRole, backendOnline, subscribeQueueRoom } = useApp();
  const navigate = useNavigate();

  useEffect(() => { setRole('patient'); }, [setRole]);

  const [selectedDocId, setSelectedDocId] = useState(myBooking?.doctor?.id ?? null);

  // Keep the selection valid as the doctors list loads/changes (e.g. the
  // moment demo data gets swapped for real backend data on connect).
  useEffect(() => {
    if (!doctors.length) return;
    const stillExists = doctors.some((d) => String(d.id) === String(selectedDocId));
    if (!stillExists) setSelectedDocId(doctors[0].id);
  }, [doctors, selectedDocId]);

  const selDoc  = doctors.find((d) => String(d.id) === String(selectedDocId)) || doctors[0];
  const myQueue = (selDoc && queues[selDoc.id]) || [];

  // Join this doctor's live Socket.io room whenever the selection changes —
  // a no-op in demo mode (subscribeQueueRoom guards on backendOnline itself).
  useEffect(() => {
    if (selDoc) subscribeQueueRoom(selDoc.id, myBooking?.token);
  }, [selDoc?.id, subscribeQueueRoom, myBooking?.token]);

  // Use real booking data if tracking the booked doctor, else demo numbers
  const myEntry = myBooking && String(myBooking.doctor?.id) === String(selDoc?.id)
  ...
  // NOTE: queue auto-advancement simulation now lives centrally in
  // AppContext and only runs while the backend is offline (demo mode).
  // This page no longer runs its own setInterval — having two separate
  // timers mutate the same queue used to advance it at double speed.

  function manualRefresh() {
    addToast(backendOnline ? 'Queue refreshed from server.' : 'Queue refreshed!', 'info');
  }
  ...
  return (
    <div className="page-wrap-lg">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        >
          <span className="w-2 h-2 rounded-full live-ring" style={{ background: backendOnline ? '#22C55E' : '#EAB308' }} />
          <span className="text-xs font-bold" style={{ color: backendOnline ? '#166534' : '#854D0E' }}>
            {backendOnline ? 'Live · Socket connected' : 'Demo Mode · Simulated updates'}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-2.5 mb-6 overflow-x-auto pb-2">
        {doctors.slice(0, 8).map((doc) => {
          ...
          const active = String(selDoc?.id) === String(doc.id);
          return (
            <button
              key={doc.id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="card p-5">
            ...
            {DEPARTMENTS.map((d) => {
              const total = doctors.filter((x) => x.dept === d.id).reduce((s, x) => s + (x.patientsLeft || 0), 0);
              return (
